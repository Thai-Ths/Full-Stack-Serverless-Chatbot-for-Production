...
# Try to read bucket names from Terraform outputs; fallback to computed names
TF_FRONTEND_BUCKET=$(terraform output -raw s3_frontend_bucket 2>/dev/null || true)
TF_MEMORY_BUCKET=$(terraform output -raw s3_memory_bucket 2>/dev/null || true)

FRONTEND_BUCKET=${TF_FRONTEND_BUCKET:-"${PROJECT_NAME}-${ENVIRONMENT}-frontend-${AWS_ACCOUNT_ID}"}
MEMORY_BUCKET=${TF_MEMORY_BUCKET:-"${PROJECT_NAME}-${ENVIRONMENT}-memory-${AWS_ACCOUNT_ID}"}
